...
void parse_args(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-h") == 0) {
            printf("Usage: minicc [options] <file.c>\n");
            printf("Options:\n");
            printf("  -b             Afficher la bannière\n");
            printf("  -o <file>      Nom du fichier de sortie (default: out.s)\n");
            printf("  -t <int>       Niveau de trace (0-5)\n");
            printf("  -r <int>       Nombre de registres (4-8)\n");
            printf("  -s             Arrêter après l'analyse syntaxique\n");
            printf("  -v             Arrêter après les vérifications sémantiques\n");
            exit(0);
        } 
        // Option -b : Bannière
        else if (strcmp(argv[i], "-b") == 0) {
            printf("Compilateur MiniCC - Etudiant : [Nom]\n");
            exit(0);
        } 
        ...
        // Option -r : Registres
            ...
            int nb_regs = atoi(argv[++i]);
            if (nb_regs >= 4 && nb_regs <= 8) {
                set_max_registers(nb_regs);
            } else {
                fprintf(stderr, "Error: -r must be between 4 and 8\n");
                exit(1);
            }
        } 
        // Option -s : Stop syntaxe
        ...
        // Fichier d'entrée (si ce n'est pas une option)
        else if (argv[i][0] != '-') {
            if (infile == NULL) {
                infile = argv[i];
            } else {
                fprintf(stderr, "Error: Multiple input files not allowed\n");
                exit(1);
            }
        }
    }
...
void free_nodes(node_t n) {
    ...
    //Libérer les chaînes allouées dans le nœud
    if (n->ident != NULL) {
        free(n->ident);
    }
    if (n->str != NULL) {
